...
void main()
{
    ...
    vec3 dir = 2.0 * real_normal * dot(real_normal, to_camera) - to_camera;
    ...
    out_color = vec4(0.5 * (lightness * albedo + texture(env_texture, vec2(x, y)).rgb), 1.0);
}
)";
...
void main()
{
    ...
    float x = atan(to_camera.z, to_camera.x) / PI * 0.5 + 0.5;
    float y = -atan(to_camera.y, length(to_camera.xz)) / PI + 0.5;

    out_color = vec4(texture(env_texture, vec2(x, y)).rgb, 1.0);
}
)";
...
int main() try {
    ...
    GLuint rec_view_location = glGetUniformLocation(rec_program, "view");
    GLuint rec_projection_location = glGetUniformLocation(rec_program, "projection");
    GLuint light_direction_location = glGetUniformLocation(program, "light_direction");
    GLuint camera_position_location = glGetUniformLocation(program, "camera_position");
    GLuint rec_camera_position_location = glGetUniformLocation(rec_program, "camera_position");
    ...
    GLuint env_texture_location = glGetUniformLocation(program, "env_texture");
    GLuint rec_env_texture_location = glGetUniformLocation(rec_program, "env_texture");
    ...
    GLuint env_texture = load_texture(project_root + "/textures/environment_map.jpg");
    ...
    while (running) {
        ...
        glUniformMatrix4fv(rec_view_location, 1, GL_FALSE, reinterpret_cast<float *>(&view));
        glUniformMatrix4fv(rec_projection_location, 1, GL_FALSE, reinterpret_cast<float *>(&projection));
        glUniform3fv(rec_camera_position_location, 1, reinterpret_cast<float *>(&camera_position));
        glUniform1i(rec_env_texture_location, 2);
