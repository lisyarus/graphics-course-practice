...
void build_bezier(std::vector<vertex> const& vertexes, int quality, std::vector<vertex>& bezier_vertexes) {
  bezier_vertexes.clear();
  int cnt = ((int)vertexes.size() - 1) * quality;
  for (int i = 0; i <= cnt; i++) {
    bezier_vertexes.push_back(vertex(
      bezier(vertexes, i / (float)cnt), { 255, 0, 0, 255 }, 0));
  }
  for (int i = 0; i < cnt; i++) {
    bezier_vertexes[i + 1].distance = bezier_vertexes[i].distance + hypot(
      bezier_vertexes[i + 1].position.x - bezier_vertexes[i].position.x,
      bezier_vertexes[i + 1].position.y - bezier_vertexes[i].position.y);
  }
}

void shift_bezier(std::vector<vertex> & vertexes, float shift) {
  for (int i = 0; i < vertexes.size(); i++) {
    vertexes[i].distance += shift;
  }
}
...
int main() try
{
  ...
  glBufferData(GL_ARRAY_BUFFER, bezier_vertexes.size() * sizeof(vertex), bezier_vertexes.data(), GL_DYNAMIC_DRAW);
  ...
  while (running)
  {
    ...
    case SDL_MOUSEBUTTONDOWN:
      if (event.button.button == SDL_BUTTON_LEFT)
      {
        ...
        vertexes.push_back(vertex(vec2(mouse_x, mouse_y), { 0, 0, 0, 255 }, 0));
        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        glBufferData(GL_ARRAY_BUFFER, vertexes.size() * sizeof(vertex), vertexes.data(), GL_DYNAMIC_DRAW);

        build_bezier(vertexes, quality, bezier_vertexes);
      }
    ...
    shift_bezier(bezier_vertexes, dt * 100);
